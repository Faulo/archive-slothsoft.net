...
$gameKey = isset($this->httpRequest->input['game'])
	? $this->httpRequest->input['game']
	: null;
		...
		if ($xpath = \Storage::loadExternalXPath($charURI, TIME_MONTH)) {
			...
			foreach ($nodeList as $node) {
				...
				foreach ($imgList as $img) {
					...
					if ($uri and $text) {
						$imageFindList = [
							['Astrid as she appears', 'Path of Radiance'],
							['Sothe as a Rogue', 'Radiant Dawn'],
							['Jagen in', 'Shadow Dragon'],
							['Lyn', 'portrait in Rekka no Ken'],
							['Eliwood', 'Tactician'],
							['Profile picture.'],
						...
						$imageNameBreaks = ['Astrid', 'Sothe', 'Jagen', 'Julian', 'Lyn', 'Eliwood'];
						foreach ($gameNames as $tmp) {
							$imageFindList[] = ['portrait', $charName, $tmp];
							$imageFindList[] = ['appears in', $charName, $tmp];
							$imageFindList[] = ['appeared in', $charName, $tmp];
						}
						//$imageFindList[] = ['portrait', $charName];
						$imageNot = ['face', 'miniportrait'];
