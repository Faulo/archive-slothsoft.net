...
if (isset($this->httpRequest->input['jlpt-level'])) {
	$attr = 'isJLPT';
	$level = isset($this->httpRequest->input['jlpt-level'])
		? $this->httpRequest->input['jlpt-level']
		: 5;
	...
	foreach ($kanjiList as $kanji) {
		...
		foreach ($kanaList as $kana) {
			...
			if (preg_match('/[\w\s]+/', $kana, $match)) {
				$node->firstChild->firstChild->setAttribute('dict-uri', 'http://www.dict.cc/?'.http_build_query(['s'=>trim($match[0])]));
			}
