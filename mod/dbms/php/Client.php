...
class Client {
	...
	public function tableExists($dbName, $tableName) {
		...
		if ($this->connect()) {
			$ret = $this->select(
				...
				'tables',
				'table_name',
				sprintf('table_schema = "%s" AND table_name = "%s"', $this->escape($dbName), $this->escape($tableName))
			);
			$ret = (bool) count($ret);
		}
		return $ret;
	...
	public function databaseExists($dbName) {
		...
		if ($this->connect()) {
			$ret = $this->select(
				...
				'schemata',
				'schema_name',
				sprintf('schema_name = "%s"', $this->escape($dbName))
			);
			$ret = (bool) count($ret);
		}
		return $ret;
	...
	public function getTableList($dbName) {
		...
		if ($this->connect()) {
			$ret = $this->select(
				...
				'tables',
				'table_name',
				sprintf('table_schema = "%s"', $this->escape($dbName))
			);
		}
		return $ret;
	}
	...
	public function optimize($dbName, $tableName) {
		...
		$err = [];
		foreach ($ret as $arr) {
			if (in_array($arr['Msg_text'], ['OK', 'Table is already up to date'])) {
				return true;
			} else {
				$err[] = $arr['Msg_text'];
			}
		...
		switch ($mode) {
			case 'db':
				$sql = sprintf(
					...
					$this->get_handle(null, $dbName), self::CONNECTION_CHARSET, self::CONNECTION_COLLATION
				);
				$this->execute($sql);
				break;
			case 'table':
				$sql = sprintf(
					...
					$this->get_handle($dbName, $tableName), self::CONNECTION_CHARSET, self::CONNECTION_COLLATION
				);
				$this->execute($sql);
				$sql = sprintf(
	...
	protected function get_ids($id) {
		if (is_array($id)) {
			switch (count($id)) {
				case 0:		return '0';
				case 1:		return sprintf('id=%d', reset($id));
				default:	return sprintf('id IN (%s)', implode(',', $id));
			}
		}
		if (is_int($id)) {
