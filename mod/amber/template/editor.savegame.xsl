...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="save:savegame.editor">
		<xsl:apply-templates select="save:archive[@file-name = $ARCHIVE_FILENAME]" mode="form"/>
	</xsl:template>
	...
	<xsl:template match="save:archive" mode="form">
		<form action="./?SaveName={../@save-id}" method="POST">
			...
			<button name="DownloadFile" type="submit" class="yellow" value="{@file-name}">Download</button>
	...
	<xsl:template match="save:archive[@file-name='Party_char.amb']" mode="form-content">
		<xsl:call-template name="savegame.tabs">
...
window.addEventListener(
	"load",
	(eve) => {
		AngularAmber.controller("character]]><xsl:value-of select="@file-name"/><![CDATA[", function($scope) {
	...
	<xsl:template match="save:archive[@file-name='NPC_char.amb']" mode="form-content">
		<xsl:call-template name="savegame.tabs">
	...
	<xsl:template match="save:archive[@file-name='Monster_char_data.amb']" mode="form-content">
		<xsl:call-template name="savegame.tabs">
	...
	<xsl:template match="save:archive[@file-name='2Map_texts.amb'] | save:archive[@file-name='3Map_texts.amb']" mode="form-content">
		<xsl:call-template name="savegame.tabs">
			<xsl:with-param name="label" select="'Aktive Karte:'"/>
	...
	<xsl:template match="save:archive[@file-name='Party_data.sav']" mode="form-content">
		<xsl:for-each select="save:file">
			<xsl:call-template name="savegame.flex">
				<xsl:with-param name="items">
					<xsl:for-each select="*">
	...
	<xsl:template match="save:archive[@file-name='Merchant_data.amb']" mode="form-content">
		<xsl:variable name="fileList" select="save:file"/>
		<xsl:call-template name="savegame.tabs">
	...
	<xsl:template match="save:archive[@file-name='Chest_data.amb']" mode="form-content">
		<xsl:variable name="fileList" select="save:file"/>
		<xsl:call-template name="savegame.tabs">
	...
	<xsl:template match="save:archive[@file-name='AM2_CPU'] | save:archive[@file-name='AM2_BLIT']" mode="form-content">
		<xsl:for-each select="save:file">
			<div>
				<xsl:call-template name="savegame.table">
	...
	<xsl:template match="save:archive[@file-name='Place_data']" mode="form-content">
		<xsl:for-each select="save:file">
			<xsl:call-template name="savegame.flex">
				<xsl:with-param name="items">
					<div>
	...
	<xsl:template match="save:archive[@file-name='CONFIG_THALION' or @file-name='CONFIG_SLOTHSOFT']" mode="form-content">
		<xsl:for-each select="save:file">
			<xsl:call-template name="savegame.flex">
				<xsl:with-param name="items">
					...
					<table>
						...
						<tbody>
							<xsl:for-each select="*/*">
								...
								<tr>
									<td class="yellow"><xsl:value-of select="position() - 1"/></td>
									<td>
	...
	<xsl:template name="savegame.amber.character-common">
		<xsl:call-template name="savegame.table">
			...
			<xsl:with-param name="items">
				<xsl:apply-templates select=".//*[@name = 'portrait']" mode="item"/>
				<xsl:apply-templates select=".//*[@name = 'name']" mode="item"/>
	...
	<xsl:template name="savegame.amber.character-ailments">
		<xsl:apply-templates select=".//*[@name = 'ailments-current']" mode="item">
...
.]]><xsl:value-of select="$id"/><![CDATA[ > *[data-picker-name="gfx-id"]::after {
	...
	background-image: url("/getResource.php/amber/monster-gfx/]]><xsl:value-of select="$gfx-id"/><![CDATA[");
	/*
...
@keyframes ]]><xsl:value-of select="$id"/><![CDATA[ {
	...
	<xsl:for-each select="$animation">
		...
		<xsl:value-of select="concat($step, '% { background-position-y: -', php:functionString('hexdec', .), 'em; } ')"/>
	</xsl:for-each>
	<![CDATA[
}
		]]></style>
			...
			<xsl:with-param name="items">
				<xsl:for-each select="$animationCycles">
					...
					<div>
						...
						<div><xsl:apply-templates select="." mode="form-content"/></div>
					</div>
				</xsl:for-each>
			</xsl:with-param>
		</xsl:call-template>
	...
	<xsl:template match="*" mode="item">
		<pre class="errorMessage">Unbekanntes form Element: <xsl:value-of select="name()"/></pre>
	</xsl:template>
			...
			<table>
				<tbody>
					<xsl:for-each select="*">
						<tr>
							<td class="yellow"><xsl:value-of select="position() - 1"/></td>
							<td><p class="gray smaller pre"><xsl:value-of select="@value"/></p></td>
						</tr>
					</xsl:for-each>
				</tbody>
			</table>
	...
	<xsl:template match="save:string | save:integer | save:signed-integer | save:select | save:binary | save:event-script" mode="item">
		<xsl:param name="class" select="''"/>
		<label>
			<xsl:apply-templates select="." mode="form-attributes">
				<xsl:with-param name="class" select="$class"/>
	...
	<xsl:template match="*" mode="form-content">
		<pre class="errorMessage">Unbekanntes form-content Element: <xsl:value-of select="name()"/></pre>
	</xsl:template>
	...
	<xsl:template match="save:group[*[@name = 'current'] and *[@name = 'current-mod'] and *[@name = 'maximum-mod']]" mode="form-content">
		<xsl:apply-templates select="*[@name = 'current']" mode="form-content"/>
		<xsl:text>+</xsl:text>
		<xsl:apply-templates select="*[@name = 'current-mod']" mode="form-content"/>
		<xsl:text>/</xsl:text>
	...
	<xsl:template match="save:group[*[@name = 'item-id']]" mode="form-content">
		...
		<div role="button" tabindex="0" class="item picker"
			...
			contextmenu="amber-picker-item" onclick="savegameEditor.openPopup(arguments[0])">
	...
	<xsl:template match="save:select" mode="form-content">
		...
		<select name="save[data][{@value-id}]">
			...
			<xsl:if test="not($node/@value = $options/@key)">
				<option value="{$node/@value}" selected="selected"><xsl:value-of select="concat('??? (', $node/@value, ')')"/></option>
			</xsl:if>
		</select>
	</xsl:template>
	...
	<xsl:template match="save:string | save:integer | save:signed-integer | save:bit | save:select" mode="form-picker">
				...
				<select class="command" disabled="disabled">
					<xsl:for-each select="$options">
						<option value="{position() - 1}">
							<xsl:if test="count(ancestor::save:file/../save:file) &gt; 1 and ancestor::save:file/@file-name != current()">
	...
	<xsl:template name="savegame.table">
		...
		<xsl:if test="$rows">
			<xsl:if test="string-length($label)">
				<h3 class="name"><xsl:value-of select="$label"/></h3>
			</xsl:if>
			<table>
				<xsl:if test="string-length($class)">
					<xsl:attribute name="class"><xsl:value-of select="$class"/></xsl:attribute>
	...
	<xsl:template name="savegame.button">
		...
		<button type="button" onclick="{$action}"><xsl:value-of select="$label"/></button>
	</xsl:template>
</xsl:stylesheet>