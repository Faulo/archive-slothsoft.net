...
<xsl:stylesheet version="1.0"
	...
	<xsl:variable name="fragment" select="/data/*[@data-cms-name = 'list']/fragment"/>
		...
		<div class="Amber Editor">
			<xsl:for-each select="$fragment/category">
				<details data-template="flex">
					<summary>
						<h2>Kategorie: <span class="green"><xsl:value-of select="@name"/></span></h2>
					</summary>
					<ul>
						<xsl:for-each select="*">
							<li><xsl:apply-templates select="." mode="itemlist"/></li>
						</xsl:for-each>
					</ul>
				</details>
			</xsl:for-each>
	...
	<xsl:template match="portrait" mode="itemlist">
		<article data-portrait-id="{@id}" data-template="flex" class="Portrait">
			<div role="button" tabindex="0" class="picker">
				<div class="PortraitId" data-dictionary="PortraitId" data-picker-value="{@id}"/>
			</div>
			...
			<ul>
				<li>
					<table class="ItemName">
						<tr>
							<td>
								<div role="button" tabindex="0" class="picker" data-hover-text="{@name}">
									<div class="ItemId" data-dictionary="ItemId" data-picker-value="{@id}"/>
								</div>
							</td>
							<td>
					...
					<table class="ItemData">
						<tbody>
							<tr class="right-aligned">
								<td>Gewicht:</td><td class="number"><xsl:value-of select="concat(@weight, ' gr')"/></td>
							</tr>
							<tr class="right-aligned">
								<td>Wert:</td><td class="number"><xsl:value-of select="concat(@price, ' gp')"/></td>
							</tr>
						</tbody>
						<tbody>
							<tr class="right-aligned">
								<td>Hände:</td><td class="number"><xsl:value-of select="@hands"/></td>
							</tr>
							<tr class="right-aligned">
								<td>Finger:</td><td class="number"><xsl:value-of select="@fingers"/></td>
							</tr>
							<tr class="right-aligned">
								<td>Schaden:</td>
								<td class="number">
							...
							<tr>
								...
								<td class="number">
									<xsl:if test="@lp-max &gt; 0">
										<xsl:choose>
											<xsl:when test="@is-cursed">-</xsl:when>
						...
						<tbody>
							...
							<tr>
								<td colspan="4">
									...
									<xsl:if test="@is-cursed"><span class="green">verflucht</span></xsl:if>
								</td>
							</tr>
						</tbody>
					</table>
