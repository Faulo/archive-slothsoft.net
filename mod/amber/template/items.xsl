...
<xsl:stylesheet version="1.0"
	...
	<xsl:variable name="itemMap" select="/data/resource[@data-cms-name = 'item-map']"/>
	
	<xsl:variable name="itemList" select="/data/data[@data-cms-name = 'items']/item"/>
	<xsl:variable name="typeList" select="/data/data[@data-cms-name = 'items']/type"/>
	...
	<xsl:template match="/data">
		<svg
			...
			>
			<defs>
				<image id="sprite-map" href="data:{$itemMap/@type};base64,{$itemMap/@base64}" width="{$itemMap/@width}" height="{$itemMap/@height}"/>
				<clipPath id="sprite-clipping">
					<rect width="{$ITEM_SIZE}" height="{$ITEM_SIZE}"/>
				</clipPath>
				<xsl:for-each select="//item">
					<use id="item-{@id}-image" href="#sprite-map" transform="translate(0, {(@image - 1) * - $ITEM_SIZE})"/>
				</xsl:for-each>
			</defs>
			<xsl:for-each select="$typeList">
				...
				<g class="{@name}" transform="translate({$typeOffset}, 0)">
					<xsl:for-each select="$itemList[@type = current()/@name]">
						...
						<use href="#item-{@id}-image" transform="translate(0, {$itemOffset})" clip-path="url(#sprite-clipping)"/>
						
						<view id="item-{@id}" viewBox="{$typeOffset} {$itemOffset} {$ITEM_SIZE} {$ITEM_SIZE}">
							<xsl:copy-of select="."/>
							<!--
							<xsl:for-each select="@*">
