...
if ($this->httpRequest->hasInputValue('css')) {
	...
	foreach ($fileList as $id => $tmp) {
		$ret[] = sprintf(
			'*[data-dictionary="PortraitId"][data-picker-value="%d"]::after { background-position-y: -%dem; }',
			$id,
			$id
		);
	}
...
$ret = [];
$ret['Menschen und Halb-Elfen ♂'] = ['human-male'];
$ret['Menschen und Halb-Elfen ♀'] = ['human-female'];
$ret['Elfen ♂'] = ['elf-male'];
$ret['Elfen ♀'] = ['elf-female'];
$ret['Zwerge und Gnome ♂'] = ['dwarf-male', 'gnome-male'];
$ret['Zwerge und Gnome ♀'] = ['dwarf-female', 'gnome-female'];
$ret['Sylphen'] = ['sylph'];
$ret['Moraner'] = ['morag'];
$ret['Tiere'] = ['cat', 'dog', 'feline'];
