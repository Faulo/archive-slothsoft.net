...
if ($this->httpRequest->hasInputValue('css')) {
    ...
    foreach ($fileNodeList as $fileNode) {
        ...
        $ret[] = sprintf('*[data-tileset="%d"] *[data-picker-name="tile-id"]::after { background-image: url(/getResource.php/amber/tilesets/%d-%d); }', $fileId, $fileId, 0);
        for ($paletteId = 0; $paletteId < $paletteCount; $paletteId ++) {
            if (file_exists(sprintf('%s/%d-%d.png', $tilesetsDir, $fileId, $paletteId))) {
                $ret[] = sprintf('*[data-tileset-icon="%d"][data-palette="%d"] *[data-picker-name="tile-id"]::after { background-image: url(/getResource.php/amber/tilesets/%d-%d); }', $fileId, $paletteId, $fileId, $paletteId);
            }
        }
        $ret[] = '';
