...
$this->httpRequest->setInputValue('save', ['editor' => [ 'archives' => ['Monster_char_data.amb']]]);
...
foreach ($monsterNodeList as $monsterNode) {
	...
	if (!file_exists($tgaPath)) {
		...
		$cmd = sprintf(
			'%s %s -out %s -w %d -b %d -p %d',
			$editor->getConfigValue('ambgfxPath'),
			escapeshellarg($spritePath),
			escapeshellarg($tgaPath),
			$spriteWidth, $spriteBitplanes, $spritePalette);
		exec($cmd);
	}
