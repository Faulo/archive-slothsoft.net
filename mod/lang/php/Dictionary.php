...
class Dictionary {
	...
	public function translateDoc(DOMDocument $doc) {
		...
		foreach ($nodeList as $node) {
			...
			$namespace = $node->hasAttribute(self::XPATH_DICT_ATTR_NS)
				? $node->getAttribute(self::XPATH_DICT_ATTR_NS)
				: null;
			$language = $node->hasAttribute(self::XPATH_DICT_ATTR_LANG)
				? $node->getAttribute(self::XPATH_DICT_ATTR_LANG)
				: null;
			$ret += $this->translateNode($xpath, $node, $attr, $namespace, $language);
		}
		$doc->documentElement->setAttribute('xml:lang', $this->getLang());
		return $ret
			? $ret + $this->translateDoc($doc)
			: 0;
	}
			...
			switch ($node->nodeType) {
				case XML_ELEMENT_NODE:
					$node->parentNode->replaceChild(
						$this->lookupFragment($replaceNode, $namespace, $language, $node->ownerDocument),
						$node
					);
					$ret++;
					break;
				...
				case XML_TEXT_NODE:
					$node->parentNode->replaceChild(
						$this->lookupFragment($node->data, $namespace, $language, $node->ownerDocument),
						$node
					);
					$ret++;
					break;
	...
	protected function getLangPath($namespace = null, $language = null) {
		...
		if (!isset($this->langPaths[$path])) {
			if (!is_file($path)) {
				...
				file_put_contents(
					$path,
					sprintf('<?xml version="1.0" encoding="UTF-8"?>%1$s<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="%2$s">%1$s%1$s</html>', self::XPATH_EOL, $language)
				);
			}
	...
	protected function addWord(DOMDocument $doc, $word, $originalWord = null) {
		...
		if (strlen($word)) {
			...
			$doc->documentElement->appendChild(
				$doc->createTextNode(self::XPATH_EOL)
			);		
