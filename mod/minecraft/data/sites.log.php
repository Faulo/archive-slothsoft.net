...
for ($year = $lastYear; $year >= $firstYear; $year--) {
	...
	$xml .= sprintf(
		'<page status-active="" status-public="" redirect="/Minecraft/Log/" name="%s" title="%s">',
		$year, $year
	);
	$startMonth = $year === $lastYear
		? $lastMonth
		: 12;
	$endMonth = $year === $firstYear
		? $firstMonth
		: 1;
	for ($month = $startMonth; $month >= $endMonth; $month--) {
		...
		$xml .= sprintf(
			'<page status-active="" status-public="" ref="archive" name="%02d" title="month/%02d">',
			$month, $month
		);
		$xml .= sprintf(
			'<param name="chat-start" value="%d"/>',
			mktime (0, 0, 0, $month, 1, $year)
		);
		$xml .= sprintf(
			'<param name="chat-end" value="%d"/>',
