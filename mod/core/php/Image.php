...
class Image {
	...
	const FFMPEG_ACTIVE = true;	//DEAKTIVIERT: temp.tmp kann nicht gelesen werden!!!!!
	//const FFMPEG_PATH = 'C:/NetzwerkDaten/Dropbox/Tools/ffmpeg/ffmpeg.exe';
	const FFMPEG_PATH = 'ffmpeg';
	const FFMPEG_QUERY_CONVERT = '-i %1$s %2$s -y';						///encode
	...
	public static function addSprite($destFile, array $sourceFileList, $spriteWidth = null, $spriteHeigh = null) {
		...
		foreach ($sourceFileList as $index => $sourceFile) {
			if ($sourceFile) {
				...
				$width = $spriteWidth === null
					? $sourceInfo['width']
					: $spriteWidth;
				$height = $spriteHeigh === null
					? $sourceInfo['height']
					: $spriteHeigh;
	...
	public static function createFromFile($file) {
		...
		switch($size['mime']){
			case 'image/jpeg':	return imagecreatefromjpeg($file); //jpeg file
			case 'image/gif':	return imagecreatefromgif($file); //gif file
			case 'image/png':	return imagecreatefrompng($file); //png file
			default: 			throw new Exception(
				sprintf('MIME TYPE NOT SUPPORTED: "%s"', basename($file))
			);
		}
	}
	...
	public static function convertFile($sourceFile, $destFile) {
		...
		if (self::IRFANVIEW_ACTIVE) {
			...
			$command.= sprintf(
				self::IRFANVIEW_QUERY_CONVERT,
				escapeshellarg($sourceFile), escapeshellarg($destFile)
			);
			exec($command);
		} elseif (self::FFMPEG_ACTIVE) {
			$command = '"' . self::FFMPEG_PATH . '" ';
			$command.= sprintf(
				self::FFMPEG_QUERY_CONVERT,
				escapeshellarg($sourceFile), escapeshellarg($destFile)
			);
			exec($command);
		}
		...
		if ($sourceFile = realpath($sourceFile)) {
			...
			if ($sourceFile and !file_exists($destFile) or filemtime($destFile) < filemtime($sourceFile)) {
				if (self::IRFANVIEW_ACTIVE and is_readable(self::IRFANVIEW_PATH)) {
					$command = '"' . self::IRFANVIEW_PATH . '" ';
					$command.= sprintf(
						self::IRFANVIEW_QUERY_SIZE,
						$sourceFile, str_replace('/', '\\', $destFile),
	...
	public static function batch() {
		...
		foreach ($files as $file) {
			...
			$arr = explode('.', $file);
			if (count($arr) === 3) unlink($dir.$file);
			continue;
			$id = $arr[0];
			$end = $arr[1];
			$arr[1] = 's';
