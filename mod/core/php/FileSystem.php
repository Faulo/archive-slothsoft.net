...
abstract class FileSystem {
	...
	public static $sizeUnits = ['B', 'kB', 'MB', 'GB', 'TB'];
	public static $videoExtensions = ['avi', 'mkv', 'mp4', 'mov', 'ogv', 'wmv', 'mpg', 'webm', 'm4v', 'flv', 'ogm'];
	public static $audioExtensions = ['mp3', 'wav', 'ogg', 'wma', 'ogm', 'mid', 'm4a'];
	public static $subttitleExtensions = ['vtt', 'srt', 'ssa', 'ass'];
	public static function isVideo($fileName) {
		return in_array(
			strtolower(self::extension($fileName)),
			...
			if ($name === 'Thumbs.db') {
				...
				if (!$retNode) {
					...
					if ($tagName) {
						...
						$attr['id'] = 'id-'.md5($path);
						$attr['name'] = $name;
						$attr['title'] = isset($attr['ext'])
							? substr($attr['name'], 0, - (1 + strlen($attr['ext'])))
							: $attr['name'];
						foreach ($attr as $key => $val) {
							$retNode->setAttribute($key, utf8_encode($val));
						}
	...
	public static function webpath($fileName, $absolute = true) {
		return $absolute
			? str_replace(DIRECTORY_SEPARATOR, '/', substr($fileName, strlen($_SERVER['DOCUMENT_ROOT'])))
			: str_replace(DIRECTORY_SEPARATOR, '/', substr($fileName, strlen(dirname($_SERVER['SCRIPT_FILENAME']).'/')));
	}
	...
	public static function lookupFile($fileName) {
		if (is_readable($fileName)) {
			...
			return is_file($fileName)
				? $com->GetFile($fileName)
				: $com->GetFolder($fileName);
	...
	public static function download($filePath, $downloadName) {
		...
		if ($start or $end !== $size) {
			...
			$headers[] = sprintf(
				'Content-Range: bytes %1$.0f-%2$.0f/%3$.0f',
				$start, $end - 1, $size
			);
		}
		$headers[] = sprintf(
			'Content-Disposition: attachment; filename*=UTF-8\'\'%1$s',
...
";
	...
	public static function downloadByURI($destPath, $sourceURI, array $options = []) {
		$ret = 'ERROR';
		$downloadCommand = isset($options['download-cmd'])
			? $options['download-cmd']
			: 'curl %s -o %s';
		$copyCommand = isset($options['copy-cmd'])
			? $options['copy-cmd']
			: 'copy %s %s /y';
		$successCommand = isset($options['success-cmd'])
			? $options['success-cmd']
			: null;
		$successPHP = isset($options['success-php'])
			? $options['success-php']
			: null;
	...
	protected static $base64Target = ['-', '_', '~'];
