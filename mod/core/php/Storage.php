...
class Storage {
	...
	public static function loadExternalHeader($uri, $cacheTime =  null, $data = null, $options = null) {
		...
		$ret = self::_randomCheck()
			? null
			: $storage->retrieveJSON($name, $storageTime);
		if ($ret === null) {
			$req = self::_httpRequest($options, $uri, $data);
	...
	public function install() {
		...
		$options = [
			//'engine' => 'MyISAM', //http://www.sitepoint.com/mysql-mistakes-php-developers/
		];
	...
	public function storeDocument($name, DOMDocument $dataDoc, $modifyTime) {
		return $dataDoc->documentElement
			? $this->store($name, $dataDoc->saveXML(), $modifyTime)
			: false;
	}
