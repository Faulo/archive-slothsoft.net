...
class DownloadWork extends Stackable {	
	...
	public function getName() {
		...
		return isset($options['chapter'])
			? sprintf('%s: %d', $options['name'], $options['chapter']) 
			: $options['name'];
	}
	/*
			...
			do {
				...
				if ($xpath) {
					if ($xpath) {
						...
						$nextURI = strlen($nextURI)
							? $this->_fixURI($nextURI, $sourceURI)
							: null;
						$image = strlen($image)
							? $this->_fixURI($image, $sourceURI)
							: null;
...
[mime] => image/jpeg
	...
	public function workIndexTool(array $options) {
		...
		if (isset($options['source-xpath'])) {
			$sourceXPathList = [$options['source-xpath']];
		} else {
			$sourceXPathList = [];
			for ($i = 0; $i < 10; $i++) {
	...
	protected function _fixFilename($name, $ext = null) {
		return $ext === null
			? FileSystem::filenameSanitize($name)
			: sprintf('%s.%s', FileSystem::filenameSanitize($name), $ext);
	}
