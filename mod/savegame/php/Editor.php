...
class Editor {
	...
	public function getGlobalById($id) {
		return isset($this->globalList[$id])
			? $this->globalList[$id]
			: null;
	}
	public function getDictionaryById($id) {
		return isset($this->dictionaryList[$id])
			? $this->dictionaryList[$id]
			: [];
	}
	...
	protected function constructValue($tagName, $type) {
		switch ($tagName) {
			...
			case 'dictionary':	return new Node\DictionaryNode();
			//values
			case 'integer':				return new Node\IntegerValue();
			case 'signed-integer':		return new Node\SignedIntegerValue();
			case 'string':		return new Node\StringValue();
			case 'bit':			return new Node\BitValue();
			case 'select':		return new Node\SelectValue();
			case 'event-script':		return new Node\EventScriptValue();
			case 'binary':		return new Node\BinaryValue();
			
			//containers
			case 'group':		return new Node\GroupContainer();
			case 'file':		return new Node\FileContainer();
			
			//instructions
			case 'bit-field':			return new Node\BitFieldInstruction();
			case 'string-dictionary':	return new Node\StringDictionaryInstruction();
			case 'event-dictionary':	return new Node\EventDictionaryInstruction();
			case 'event':				return new Node\EventInstruction();
			case 'event-step':			return new Node\EventStepInstruction();
			case 'for-each-file':		return new Node\ForEachFileInstruction();
			case 'repeat-group':		return new Node\RepeatGroupInstruction();
			case 'use-global':			return new Node\UseGlobalInstruction();
			
			//archive types
			case 'archive':
				switch ($type) {
					case 'AMBR':	return new Node\ArchiveNodeAMBR();
					case 'JH':		return new Node\ArchiveNodeJH();
					case 'Raw':		return new Node\ArchiveNodeRaw();
					case 'AM2':		return new Node\ArchiveNodeAM2();
					default:		return new Node\ArchiveNode();
				}
			default:
