<?php
/***********************************************************************
 * \CMS\HTTPDocument v1.00 19.10.2012 © Daniel Schulz
 * 
 * 	Changelog:
 *		v1.00 19.10.2012
 *			initial release
...
class HTTPDocument {
	...
	protected static function normalizeNode(DOMNode $sourceNode, DOMDocument $retDoc, array $nsList) {
		...
		switch ($sourceNode->nodeType) {
			...
			case XML_ELEMENT_NODE:
				$tagName = isset($nsList[$sourceNode->namespaceURI])
					? $nsList[$sourceNode->namespaceURI] . ':' . $sourceNode->localName
				...
				foreach ($sourceNode->attributes as $childNode) {
					$tagName = strlen($childNode->prefix)
						? $childNode->prefix . ':' . $childNode->localName
						: $childNode->localName;
	...
	private $session;
	private $dataDocs 		= [];		//DOMDocument[]
	private $templateDocs 	= [];		//DOMDocument[]
	private $fragmentDocs 	= [];		//DOMDocument[]
	private $resourceDocs 	= [];		//DOMDocument[]
	private $resourceDirs 	= [];		//DOMDocument[][]
	private $sitesDoc;					//DOMDocument
	private $sitesPath;					//DOMXPath
	private $moduleRoot;				//DOMElement <modules>
	private $requestElement;			//DOMElement <request>
	private $requestedDomain;			//DOMElement
	private $requestedPage;				//DOMElement
	private $requestedNS;				//string
	private $currentElement;			//DOMElement
	private $impl;						//DOMImplementation
	...
	private $redirectUpAgents 	= ['Googlebot', 'bingbot'];
	...
	public function addBanned($ip = null) {
		...
		$this->setBannedList(array_merge($this->getBannedList(), [$ip]));
	}
	public function removeBanned($ip = null) {
		if (!$ip) {
			$ip = $this->httpRequest->clientIp;
		}
		$this->setBannedList(array_diff($this->getBannedList(), [$ip]));
	}
	...
	private function lookupDomain($domain) {
		$nodeList = $this->sitesPath->evaluate(
			sprintf('%s[@default][1]', self::TAG_DOMAIN),
			$this->sitesDoc->documentElement
		);
		foreach ($nodeList as $node) {
			$this->requestedDomain = $node;
		}
		$nodeList = $this->sitesPath->evaluate(
			sprintf('%s[@name = "%s"][1]', self::TAG_DOMAIN, $domain),
			$this->sitesDoc->documentElement
		);
		foreach ($nodeList as $node) {
			$this->requestedDomain = $node;
	...
	private function loadModule($module) {
		...
		$this->moduleRoot->appendChild(
			$this->sitesDoc->importNode($doc->documentElement, true)
		);
	...
	private function findUri(DOMElement $pageNode, $includeDomain = false) {
		...
		return $includeDomain
			? $this->httpRequest->schema . '://' . $this->requestedDomain->getAttribute('name') . $ret
			: $ret;
	}
		...
		if (!isset($this->fragmentDocs[$path])) {
			if ($resNode = $this->getModuleNode($path, self::TAG_FRAGMENT)) {
				...
				if ($fragment = $this->parseFragment($resNode)) {
					...
					$parentModule = $this->getOwnerModule(
						$templateNode
							? $templateNode
							: $resNode
					);
				...
				$dataDoc->documentElement->appendChild(
					$dataDoc->createTextNode(
						sprintf('Could not find module or fragment "%s"!', $path)
					)
				);
			}
		...
		if ($templateDoc) {
			...
			foreach ($nodeList as $node) {
				...
				if (strpos($href, '/getTemplate.php') === 0) {
					...
					$node->setAttribute(
						'href',
						'../../' . $xslNS . '/' . self::DIR_TEMPLATE . $xslName . '.xsl'
					);
