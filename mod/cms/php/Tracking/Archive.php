...
class Archive {
	...
	public function updateOutdated(array $idList) {
		$this->tempTable->update(['version' => $this->config['version']], $idList);
	}
	...
	protected function fixTemp() {
		...
		while ($idList = $this->getOutdated()) {
			$rowList = $this->tempTable->select(['id', 'data'], ['id' => $idList]);
			foreach ($rowList as $row) {
				...
				if (is_array($data)) {
					...
					if ($update) {
						$this->tempTable->update(['data' => json_encode($data)], $id);
					}
				}
			}
			$ret += count($idList);
				...
				if (is_array($data)) {
					...
					switch ($status) {
						...
						case self::PREPARE_UPDATE:
							...
							$this->tempTable->update(['data' => $row['data']], $id);
							$table->insert($id, $row['data'], $data);
							break;
						//delete
