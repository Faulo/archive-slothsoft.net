...
class Tick {
	...
	protected function append() {
		...
		if ($mode) {
			...
			$func = isset($backtrace['class'], $backtrace['type'])
				? $backtrace['class'] . $backtrace['type'] . $backtrace['function']
				: $backtrace['function'];
			$this->timelineList[] = sprintf(
				'%6dms (+%4dms) & %5dMB to get to %s',
				$time, $time - $this->lastTime, memory_get_peak_usage() / 1048576, $func
			);
			$this->lastTime = $time;
		} else {
