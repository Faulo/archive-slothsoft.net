...
class Session {
	...
	protected function install() {
		...
		$sqlKeys = [
			...
			'session',
			'name',
			['type' => 'UNIQUE KEY', 'columns' => ['session', 'name'], 'name' => 'session-name'],
			'create-time',
			'modify-time',
			'access-time',
		];
	...
	protected function loadData($session, $key, $val) {
		...
		$res = $this->dbmsTable
			? $this->dbmsTable->select('data', $arr)
			: null;
		return $res
			? $this->decodeData(current($res), true)
			: $val;
	}
	protected function saveData($session, $key, $val) {
		...
		if ($this->dbmsTable) {
			if ($idList = $this->dbmsTable->select('id', $arr)) {
				try {
					$this->dbmsTable->update(['data' => $this->encodeData($val)], $idList);
				} catch(Exception $e) {
				}
			} else {
