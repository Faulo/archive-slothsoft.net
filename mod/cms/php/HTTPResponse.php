<?php
/***********************************************************************
 * \CMS\HTTPResponse v1.00 19.10.2012 © Daniel Schulz
 * 
 * 	Changelog:
 *		v1.00 19.10.2012
 *			initial release
...
";
	...
	const BODY_STRING 	= 1;	//body = output string
	const BODY_FILE		= 2;	//body = path to file
	const BODY_STREAM	= 3;	//body = HTTPStream
	const BODY_COMMAND	= 4;	//body = HTTPCommand
	...
	public function addHeader($key, $val, $param = null) {
		...
		$this->headerList[$key] = $param === null
			? $val
			: vsprintf($val, $param);
	}
	...
	public function setRange($range) {
		if (preg_match('/^bytes=(\d*)-(\d*)(.*)$/', $range, $match)) {
			if ($match[3]) {
				...
				$this->rangeStart = strlen($match[1])
					? (float) $match[1]
					: null;
				$this->rangeEnd = strlen($match[2])
					? (float) $match[2] + 1
					: null;
			}
		}
	...
	public function setDownload($isDownload) {
		$this->fileDisposition = $isDownload
			? 'attachment'
			: 'inline';
	}
	public function setRedirect($uri, $permanent = false, $forceGET = false) {
		$this->setStatus(
			$forceGET
				...
				: ($permanent ? self::STATUS_PERMANENT_REDIRECT : self::STATUS_TEMPORARY_REDIRECT)
		);
...
EOT;
	...
	public function setDocument(DOMDocument $doc) {
		...
		if ($this->includeStyle and $this->styleFiles) {
			if (self::$httpConfig['merge-styleFiles']) {
				...
				foreach ($this->styleFiles as $path => &$styleFile) {
					...
					$styleFile = $this->getMergedStyleFile([$styleFile]);
				}
				unset($styleFile);
			}
			...
			foreach ($this->styleFiles as $styleFile) {
				$doc->insertBefore(
					$doc->createProcessingInstruction(
						'xml-stylesheet',
						sprintf('type="text/css" href="%s"', $styleFile)
					),
					$doc->firstChild
				);
			}
		...
		if ($this->scriptFiles) {
			if (self::$httpConfig['merge-scriptFiles']) {
				...
				foreach ($this->scriptFiles as $path => &$scriptFile) {
					...
					$scriptFile = $this->getMergedScriptFile([$scriptFile]);
				}
				unset($scriptFile);
			}
		}
		...
		switch ($ns) {
			...
			case HTTPDocument::NS_SVG:
				...
				if ($head = $doc->getElementsByTagNameNS(HTTPDocument::NS_SVG, 'defs')->item(0)) {
				} else {
					$head = $doc->documentElement;
				}
				foreach ($this->scriptFiles as $scriptFile) {
	...
	protected function sendHeaderList() {
		...
		if ($this->includeBody or $this->method === HTTPRequest::METHOD_HEAD) {
			$this->addHeader('content-type', '%s; charset=%s', [$this->mime, $this->charset]);
			...
			$this->addHeader(
				...
				[$this->fileDisposition, preg_replace('/[^[:print:]]/', '', $file), rawurlencode($file)]
			);
			if ($this->rangeEnd !== null) {
				...
				if ($this->status === self::STATUS_PARTIAL_CONTENT or $this->status === self::STATUS_REQUESTED_RANGE_NOT_SATISFIABLE) {
					$this->addHeader('content-range', 'bytes %1$.0f-%2$.0f/%3$.0f', [$this->rangeStart, $this->rangeEnd - 1, $this->bodyLength]);
				}
			}
		...
		switch ($this->bodyType) {
			...
			case self::BODY_FILE:
				...
				$this->addHeader('cache-control', 'must-revalidate, max-age=%d', [$cacheDuration]);
				break;
		...
		header(
			sprintf(
				...
				$this->protocolMinorVersion,
				$this->status,
				self::$httpStatusCodes[$this->status]
			),
			true,
			$this->status
		);
