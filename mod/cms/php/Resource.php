<?php
/***********************************************************************
 * \CMS\Resource v1.02 05.01.2015 © Daniel Schulz
 * 
 * 	Changelog:
 *		v1.02 05.01.2015
 *			loadMode['thumbnail']
...
class Resource {
	...
	protected static $loadedMimes = [];	//DOMElement[]
	...
	public static function getResourceDir($path, DOMElement $resNode, $loadFile = '') {
		...
		if (is_dir($realpath)) {
			$pattern = $resNode->hasAttribute('pattern')
				...
				: false;
			$extension = $resNode->hasAttribute('ext')
				? $resNode->getAttribute('ext')
				: (string) self::getExtension($resNode->getAttribute('type'));
			if (strlen($extension)) {
				$extension = '/\.'.$extension.'$/i';
			} else {
				$extension = false;
			}
			$arr = FileSystem::scanDir(
				$realpath,
				FileSystem::SCANDIR_EXCLUDE_DIRS | FileSystem::SCANDIR_EXCLUDE_HIDDEN
			);
			foreach ($arr as $file) {
	...
	protected static function instantiateResource($mimeType) {
		if (isset(self::$loadedMimes[$mimeType]) or self::loadMime($mimeType)) {
			switch ($mimeType) {
				case 'text/html':				return new ResourceHTML();
				case 'image/svg+xml':
				case 'application/xhtml+xml':
				case 'application/rdf+xml':
				case 'application/xml':			return new ResourceXML();
				case 'application/json':		return new ResourceJSON();
				case 'application/x-nbt':		return new ResourceNBT();
				case 'application/x-mcr':		return new ResourceMCRegion();
				case 'application/x-mca':		return new ResourceMCAnvil();
				case 'application/x-ttf':		return new ResourceFont();
				case 'text/plain':				return new ResourceText();
				case 'text/csv':				return new ResourceCSV();
				case 'image/*':
				case 'image/png':				return new ResourceImage();
	...
	protected function init($path, DOMElement $resNode, $loadFile) {
		...
		foreach ($options as $opt) {
			foreach ($opt->attributes as $attr) {
				$this->options[$attr->name] = $resNode->hasAttribute($attr->name)
					? $resNode->getAttribute($attr->name)
					: $attr->value;
			}
		}
	...
	protected function loadFileStatus() {
		...
		//*
		foreach (['a' => 'access', 'm' => 'change', 'c' => 'inode'] as $key => $attr) {
	...
	protected function getContent() {
		return $this->fileData === null
			? $this->fileData = file_get_contents($this->getPath())
			: $this->fileData;
	}
