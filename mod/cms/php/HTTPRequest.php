...
class HTTPRequest {
	...
	public static function prepareEnvironment(array &$env) {
		...
		if (isset($env['HTTP_ACCEPT_LANGUAGE'])) {
			if ($matchList = http_parse_language($env['HTTP_ACCEPT_LANGUAGE'])) {
				...
				if ($lang) {
					$lang = (isset($lang['region']) and strlen($lang['region']))
						? sprintf('%s-%s', $lang['language'], $lang['region'])
						: $lang['language'];
				}
			}
		}
		...
		$key = isset($env['HTTP_HOST'])
			? array_search(strtolower($env['HTTP_HOST']), self::$allowedHostList)
			: false;
		if ($key === false) {
			$key = array_search(strtolower($env['SERVER_NAME']), self::$allowedHostList);
		}
		$env['SERVER_NAME'] = $key === false
			? self::$allowedHostList[0]
			: self::$allowedHostList[$key];
	...
	public function init(array $env) {
		$this->method 		= isset($env['REQUEST_METHOD'])
			? $env['REQUEST_METHOD']
			: self::METHOD_GET;
		$this->schema = isset($env['REQUEST_SCHEME'])
			? $env['REQUEST_SCHEME']
			: self::PROTOCOL_HTTP;
		$this->schema		= strtolower($this->schema);
		$protocol = isset($env['SERVER_PROTOCOL'])
			? $env['SERVER_PROTOCOL']
			: 'HTTP/1.1';
		...
		$this->time 		= isset($env['REQUEST_TIME'])
			? $env['REQUEST_TIME']
			: time();
		$this->timeFloat 	= isset($env['REQUEST_TIME_FLOAT'])
			? $env['REQUEST_TIME_FLOAT']
			: (float) time();
		$this->clientIp 	= isset($env['REMOTE_ADDR'])
			? $env['REMOTE_ADDR']
			: '127.0.0.1';
		$this->clientAgent	= isset($env['HTTP_USER_AGENT'])
			? $env['HTTP_USER_AGENT']
			: '';
		$this->clientHost	= isset($env['HTTP_HOST'])
			? $env['HTTP_HOST']
			: SERVER_NAME;
