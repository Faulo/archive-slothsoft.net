...
class DataTable extends Table {
	...
	protected function cast(array &$data) {
		foreach (['id', 'parent_id', 'type', 'namespace', 'name', 'position'] as $key) {
			if (isset($data[$key])) {
	...
	public function cleanse($documentId) {
		while ($idList = $this->dbmsTable->select('id', ['parent_id' => null], sprintf(' AND id != %d', $documentId))) {
			...
			$this->dbmsTable->update(['parent_id' => null], $childIdList);
		}
	}
	...
	public function getNode($id) {
		$dataList = $this->dbmsTable->select(true, ['id' => $id]);
		return $dataList
			? $this->cast($dataList[0])
			: null;
	}
	public function getIdByAttribute($attrValue, array $attrIdList) {
		$idList = $this->dbmsTable->select('parent_id', ['value' => $attrValue], sprintf('AND parent_id IS NOT NULL AND name IN (%s) LIMIT 1', implode(',', $attrIdList)));
		return count($idList)
			? (int) $idList[0]
			: null;
	}
	public function getIdListByAttribute($attrValue, array $attrIdList) {
		$idList = $this->dbmsTable->select('parent_id', ['value' => $attrValue], sprintf('AND parent_id IS NOT NULL AND name IN (%s)', implode(',', $attrIdList)));
	...
	public function getNodeListByParent($parentId) {
		$dataList = $this->dbmsTable->select(true, ['parent_id' => $parentId]);
		return $this->castList($dataList);
	}
	public function getChildNodes($parentId) {
		$dataList = $this->dbmsTable->select(true, ['parent_id' => $parentId], 'AND type != ' . Node::ATTRIBUTE_NODE);
		return $this->castList($dataList);
	}
	public function getAttributes($parentId) {
		$dataList = $this->dbmsTable->select(true, ['parent_id' => $parentId, 'type' => Node::ATTRIBUTE_NODE]);
		return $this->castList($dataList);
	}
	public function getNodeByType($type) {
		$dataList = $this->dbmsTable->select(true, ['type' => $type], 'LIMIT 1');
		return $dataList
			? $this->cast($dataList[0])
			: null;
	}
	public function getNodeListByType($type) {
		return $this->dbmsTable->select(true, ['type' => $type]);
	}
