...
class Archive {
	...
	protected function install() {
		...
		$sqlKeys = [
			...
			'version',
			'tweet_time',
			['type' => 'UNIQUE KEY', 'columns' => ['tweet_id']],
		];
		$this->dbmsTable->createTable($sqlCols, $sqlKeys);
	}
	public function getTextList(array $options) {
		$limit = $options['limit'] > 0
			? sprintf('1 LIMIT %d', $options['limit'])
			: '1';
		return $this->dbmsTable->select(
			'tweet_text',
			$limit
		);
	}
	public function asNode(DOMDocument $dataDoc, array $options) {
		...
		$offset = $options['offset'] > 0
			? sprintf('OFFSET %d', $options['offset'])
			: '';
		$dataList = $this->dbmsTable->select(
			['tweet_id', 'tweet_text', 'tweet_time', 'tweet_media'],
	...
	public function upgrade() {
		...
		do {
			if ($tweetList = $this->dbmsTable->select(
				['id', 'tweet_id', 'tweet_html'],
				sprintf('id > %d AND version < %d', $lastId, self::VERSION),
				sprintf('ORDER BY id LIMIT %d', $max))) {
	...
	protected function parseTweet($tweetId, $tweetHTML, $isHTML) {
		...
		if ($tweetNode = $this->dom->parse($tweetHTML, $this->doc, $isHTML)) {
			if ($textNode = $this->xpath->evaluate('.//*[@lang] | .//p', $tweetNode)->item(0)) {
				...
				$tweet['tweet_html'] = $isHTML
					? $this->dom->stringify($tweetNode)
					: $tweetHTML;
				...
				foreach ($nodeList as $node) {
					$node->parentNode->replaceChild(
						$node->ownerDocument->createTextNode($node->getAttribute('alt')),
						$node
					);			
				}
	...
	protected function downloadImage($href) {
		...
		if (strpos($file, '.')) {
			...
			$path = ROOT . self::DIR_IMAGES . $file;
	...
	protected function downloadHTML($href) {
		...
		$path = ROOT . self::DIR_HTML . $file . '.html';
