...
class Model {
		...
		if ($this->dbmsTable) {
			if (strlen($message) and $time and strlen($ip)) {
				...
				$this->dbmsTable->insert(
					array('time' => $time, 'message' => $message, 'ip' => $ip, 'type' => Log::$messageTypes['rcon'])
				);
			}
		}
	}
	...
	public function getFirstTime() {
		...
		if ($this->dbmsTable) {
			$res = $this->dbmsTable->select(
				'time',
				'time > 0 ORDER BY time ASC LIMIT 1'
			);
			$ret = count($res)
				? reset($res)
				: 0;
	...
	public function getRangeNode($start, $end, DOMDocument $dataDoc) {
		...
		if ($this->dbmsTable) {
			...
			$messageList = $this->dbmsTable->select(
				true,
				sprintf(
					'type IN (%s) AND time > %d AND time <= %d ORDER BY time ASC',
					implode(',', $messageTypes),
					$start,
					$end
				)
	...
	protected function parseText($text, DOMDocument $doc) {
		...
		while (preg_match('/(^.*?)(https?:\/\/[^ ]+)(.*$)/i', $text, $match)) {
			if (strlen($match[1])) {
				$retFragment->appendChild(
					$this->createText($match[1], $doc)
				);
			}
	...
	protected function calcColor($color) {
		if (!isset($this->colorCache[$color])) {
			$this->colorCache[$color] = [255,255,255];
			$md5 = md5($color);
			for ($i = 0; $i < 12; $i += 2) {
				$key = (int) ($i / 4);
