...
class InterfaceGen {
	...
	protected $phpProlog = '<?php';	//chr(239) . chr(187) . chr(191);
	...
	protected function writeInterfaces() {
		...
		foreach($this->interfaceList as $interface) {
			...
			if ($interface['attr']) {
				foreach ($interface['attr'] as $code) {
					$code = str_replace(';', implode(PHP_EOL, [' {', "\t", '}']), $code);
					$codeList[] = '';
					$codeList[] = "\t" . str_replace(PHP_EOL, PHP_EOL . "\t", $code);
				}
			}
			if ($interface['func']) {
				foreach ($interface['func'] as $code) {
					...
					$code = preg_replace(
						['/\(([[:alpha:]]+) /', '/, ([[:alpha:]]+) /'],
						['('.preg_quote($interfaceBase).'$1 ', ', '.preg_quote($interfaceBase).'$1 '],
						$code
					);
					$codeList[] = '';
					$codeList[] = "\t" . str_replace(PHP_EOL, PHP_EOL . "\t", $code);
					...
					if (strlen($text)) {
						if (preg_match('/([\w\s]+)(.*)/', $text, $match)) {
							...
							if (isset($attr['const'])) {
								...
								$interface['attr'][] = $this->createFunction(
									'get' . $func,
									$type,
									[],
									$setException
								...
								if (preg_match('/\((.*?)\)/', $right, $match)) {
									...
									$interface['func'][] = $this->createFunction(
										$name,
										$type,
										$paramList,
										$exception
