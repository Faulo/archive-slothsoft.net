...
class DOMDocumentSmart extends DOMDocument {
	...
	const TEMPL_SIGN = '##';		//Zeichen für Template Platzhalter Begin/Ende
	const TEMPL_SIGN_LENGTH = 2;	//Länge des Template-Zeichens
	const TEMPL_LIST = 'tm_list';	//Attribute für Listen-Templates
	const TEMPL_COND = 'tm_cond';	//Attribute für if-statements
	...
	public static $entity_keys = array();	//Gegenstück zu $entity_list, um &entities; zu UTF-8 zu konvertieren
	...
	public function saveHTML($keepTemplate = false) {
		if (!$keepTemplate) {
			$removeAttributeExceptions = array('value', 'title', 'alt');	//bei diesen Attributen wird nur ## entfernt
			$TextNodes = array();
			$AttrNodes = array();
		...
		switch (true) {
			...
			default:	//HTML Parser anwerfen
				...
				if (strlen($Input)) {
					try {//XML probieren
						$xml = sprintf(
							'<?xml version="1.0" encoding="%s"?><xml>%s</xml>',
							self::DEFAULT_CHARSET, strtr($Input, self::$entity_keys)
						);
	...
	public static function importLanguage(DOMDocument $doc, $file, $lang_id) {
		...
		$lang_ids = array_unique(array_merge(array($lang_id, $_LANG_DFLT), array_keys($_LANG['str'])));
		$lang_file = null;
	...
	private function proccessTemplateNode($Root, array &$replaceNodes, $depthStack = array()) {		
		...
		foreach ($ListNodes as $ListNodeRef) {
			...
			for ($i = $ListNodeStart, $ListNodeCount = 0, $ListNodeContinue = true;
				$ListNodeContinue;
				$i += $ListNodeStep, $ListNodeCount++) {
		...
		foreach ($AttrNodes as $arr) {
			...
			$this->replaceContent($arr[0], array($arr[1], $this->getReplaceNode($replaceNodes, $key, $depthStack)));
		}
			...
			if ($Root->hasAttributes()) {
				foreach ($Root->attributes as $Attr) {
					...
					while ((($start = strpos($Attr->value, self::TEMPL_SIGN, $start)) !== false) && (($end = strpos($Attr->value, self::TEMPL_SIGN, $start+self::TEMPL_SIGN_LENGTH)) !== false)) {
						$end += self::TEMPL_SIGN_LENGTH;
						$AttrNodes[] = array($Attr, substr($Attr->value, $start, $end - $start));
						$start = $end;
					}
				}
			}
	...
	public static function saveXHTML(DOMDocument $doc, DOMNode $node = null) {
		$selfTerminate = array('area','base','basefont','br','col','frame','hr','img','input','link','meta','param');
		if ($node === null) {
	...
	public static function transform($xmlDoc, $xslDoc, $returnAsString = false) {
		...
		return $returnAsString
			? $xslt->transformToXML($xmlDoc)
			: $xslt->transformToDoc($xmlDoc);
	}
	...
	public static function getTextFromNode(DOMNode $Node, $showComments = false) {    
		if (in_array($Node->nodeType, $showComments
				? array(XML_TEXT_NODE, XML_CDATA_SECTION_NODE, XML_COMMENT_NODE)
