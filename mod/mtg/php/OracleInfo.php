...
class OracleInfo
{
    ...
    {
        return sprintf('%smod/mtg/res/images/set-%s/%s', ROOT, $card['expansion_abbr'], self::getCardImageName($card));
    }
    ...
    {
        return sprintf('%smod/mtg/res/images/set-%s/%s-%s.png', ROOT, $card['expansion_abbr'], $card['expansion_abbr'], substr(strtolower($card['rarity']), 0, 1));
    }

    public static function getColorPath(array &$card)
    {
        return sprintf('%smod/mtg/res/images/color.%s.png', ROOT, $card['color']);
    }
}