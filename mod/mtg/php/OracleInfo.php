...
class OracleInfo {
	...
	public static function getSetFragment(DOMDocument $dataDoc) {
		...
		if (!$retFragment) {
			...
			foreach ($setList as $set) {
				...
				$xml .= sprintf('<set href="%s" date="%s" date-year="%s" date-month="%s" date-day="%s" type="%s">%s</set>',
				$set['href'], $set['date'], date('Y', $time), date('m', $time), date('d', $time), $set['type'], $set['name']
			);
			}
			$retFragment = self::domHelper()->parse($xml);
		}
					...
					foreach ($cellNodeList as $cellNode) {
						...
						if (strlen($set) and strlen($type)) {
							switch ($type) {
								...
								default:
									throw new Exception(
										sprintf('Unknown set type? "%s"', $type)
									);
							}
						}
					}
	...
	public static function translateSetName($setName) {
		return isset(self::$_setTranslations[$setName])
			? self::$_setTranslations[$setName]
			: $setName;
	}
	
	//Type...
	public static function getCardTypeName(array &$card) {
		$type = strpos($card['type'], 'Token') === false
			? $card['type']
			: 'Token';
		foreach (self::$_typeList as $name) {
	...
	public static function getOracleURL(array &$card) {
		return $card['oracle_id'] > 0
			? sprintf(self::URL_ORACLE, $card['oracle_id'])
			: null;
	}
	...
	public static function getPriceURL(array &$card) {
		return $card['oracle_id'] > 0
			? sprintf(self::URL_PRICE, rawurlencode($card['name']))
			: null;
	}
	...
	public static function getImageURL(array &$card) {
		...
		//*/
		return sprintf(
			'/getData.php/mtg/image-card?name=%s',
			rawurlencode($card['name'])
		);
	}
	public static function getImagePath(array &$card) {
		return sprintf(
			'%smod/mtg/res/images/set-%s/%s',
			ROOT, $card['expansion_abbr'], self::getCardImageName($card)
		);
	}
	public static function getRarityURL(array &$card) {
		return sprintf(
			'/getData.php/mtg/image-rarity?expansion_abbr=%s&rarity=%s',
			$card['expansion_abbr'], $card['rarity']
		);
	}
	public static function getRarityPath(array &$card) {
		return sprintf(
			'%smod/mtg/res/images/set-%s/%s-%s.png',
			ROOT, $card['expansion_abbr'], $card['expansion_abbr'], substr(strtolower($card['rarity']), 0, 1)
		);
	}
	public static function getColorPath(array &$card) {
		return sprintf(
			'%smod/mtg/res/images/color.%s.png',
			ROOT, $card['color']
		);
	}
}