...
class Oracle implements Serializable {
	...
	private $_oracleSetBlacklist = ['Promo set for Gatherer'];
	...
	public function createSearchElement(DOMDocument $dataDoc, $inputQuery, $stock = 1) {
		if (is_array($inputQuery)) {
			...
			if (preg_match('/(.+)#(.+)/', $inputQuery, $match)) {
				...
				$query = ['name' => $inputQuery];
			}
		}
		$query = array_merge(['name' => '', 'type' => '', 'rarity' => '', 'expansion_name' => '', 'description' => '', 'cost' => '', 'legality' => '', 'cmc' => '', 'expansion_abbr' => '', 'expansion_number' => ''], $query);
	...
	public function createCardElement(DOMDocument $dataDoc = null, $name = '') {
		...
		$ret = null;
		$xmlTable = $this->getXMLTable();
		if ($xml = $xmlTable->getXMLListByNameList([$name])) {
			$xml = implode('', $xml);
	...
	public function serialize() {
		return serialize(['db' => $this->dbName]);
	}
