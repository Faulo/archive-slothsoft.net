...
$mapping['description'] = 'ORACLE';
		...
		$sqlKeys = [
			'id',
			['type' => 'UNIQUE KEY', 'columns' => ['oracle_id']],
			'name',
			'type',
			'cost',
			'rarity',
	...
	public function getSetList() {
		...
		$res = $this->dbmsTable->select(['expansion_abbr', 'expansion_name', 'count(id) cards'], '', 'GROUP BY expansion_abbr ORDER BY cards DESC');
		foreach ($res as $row) {
	...
	public function getMissingOracleIdList(array $idList) {
		return array_diff(
			$idList,
			$this->dbmsTable->select('oracle_id', sprintf('oracle_id IN (%s)', implode(',', $idList)))
		);
	}
	public function getNameListByOracleIdList(array $idList) {
		$nameList = $this->dbmsTable->select('name', ['oracle_id' => $idList]);
		return $nameList;
	}
	public function getNameByOracleId($id) {
		$nameList = $this->dbmsTable->select('name', ['oracle_id' => $id]);
		return current($nameList);
	}
	public function getNameListBySetName($setName) {
		return $this->dbmsTable->select('name', ['expansion_name' => $setName], 'ORDER BY expansion_index');
	}
	public function getIdListByName($name) {
		return $this->dbmsTable->select('id', ['name' => $name]);
	}
	public function getCardListBySetName($setName) {
		return $this->dbmsTable->select(true, ['expansion_name' => $setName], 'ORDER BY expansion_index');
	}
	public function getCardListBySetAbbr($setAbbr) {
		return $this->dbmsTable->select(true, ['expansion_abbr' => $setAbbr], 'ORDER BY expansion_index');
	}
	public function getCardByName($name) {
		$ret = $this->dbmsTable->select(true, ['name' => $name], 'ORDER BY oracle_id DESC');
		return $ret
			? reset($ret)
			: null;
	}
	public function getCardListByName($name) {
		return $this->dbmsTable->select(true, ['name' => $name]);
	}
	public function getCardListByNameList(array $nameList) {
		return $this->dbmsTable->select(true, ['name' => $nameList]);
	}
	public function getUniqueCardList() {
		$retList = [];
		$nameList = $this->dbmsTable->select('DISTINCT name');
		foreach ($nameList as $name) {
			if ($resList = $this->dbmsTable->select(true, ['name' => $name], 'ORDER BY oracle_id DESC')) {
				$card = reset($resList);
				$card['expansion_list'] = [];
				$card['legality_list'] = [];
