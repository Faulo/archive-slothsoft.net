...
class OracleWork extends Stackable {
	...
	public function work() {
		...
		switch ($options['mode']) {
			case 'start':			return $this->workStart($options);
			case 'end':				return $this->workEnd($options);
			
			case 'review_cfb':		return $this->workReviewCFB($options);
			
			case 'index_custom':	return $this->workIndexCustom($options);
			case 'index_spoiler':	return $this->workIndexSpoiler($options);
			case 'index_tokens':	return $this->workIndexTokens($options);
			case 'index_gatherer':	return $this->workIndexGatherer($options);
			case 'index_convert':	return $this->workIndexConvert($options);
			case 'index_images':	return $this->workIndexImages($options);
			case 'index_prices':	return $this->workIndexPrices($options);
			
			//case 'oracle_import':	return $this->workOracleImport($options);
			case 'oracle_set':		return $this->workOracleSet($options);
			//case 'oracle_price':	return $this->workOraclePrice($options);
			case 'oracle_xml':		return $this->workOracleXML($options);
			...
			case 'custom_card':		return $this->workCustomCard($options);
			case 'custom_cardlist':	return $this->workCustomCardList($options);
			case 'custom_import':	return $this->workCustomImport($options);
			case 'download_images':	return $this->workDownloadImages($options);
			case 'download_prices':	return $this->workDownloadPrices($options);
	...
	protected function workReviewCFB(array $options) {
		...
		$reviewFile = sprintf(
			'%s%s%s.json',
			$options['reviewDir'], DIRECTORY_SEPARATOR, $options['reviewName']
		);
	...
	protected function workIndexCustom(array $options) {
		...
		if ($setDir) {
			...
			foreach ($fileList as $file) {
				...
				foreach ($cardNodeList as $cardIndex => $cardNode) {
					...
					$isArtifact = strpos($card['type'], 'Artifact') === false
						? ''
						: 'artifact';
					$isCreature = strpos($card['type'], 'Creature') === false
						? ''
						: 'creature';
					
					$query = sprintf(
						'~%s[a-z ]* %s %s %s token~',
						preg_quote($pt), preg_quote($types), preg_quote($isArtifact), preg_quote($isCreature)
					);
		...
		$this->log(
			sprintf(
				'IndexCustom: Prepared %d custom set threads!!',
				count($ret)
			)
		);
		return $ret;
	...
	protected function workIndexSpoiler(array $options) {
		...
		foreach ($setList as $set) {
			...
			if ($xpath = Storage::loadExternalXPath($setURL, 0)) {
				...
				foreach ($cardNodeList as $cardNode) {
					...
					$rarity = isset($rarityList[$rarity])
						? $rarityList[$rarity]
						: 'Common';
		...
		$this->log(
			sprintf(
				'IndexSpoiler: Prepared %d spoiled cards!!',
				count($cardList)
			)
		);
		return $ret;
	...
	protected function workIndexSpoilerOLD(array $options) {
		...
		$this->log(
			sprintf(
				'IndexSpoiler: Prepared %d custom set threads!!',
				count($ret)
			)
		);
		return $ret;
	...
	protected function workIndexTokens(array $options) {
		...
		$this->log(
			sprintf(
				'IndexTokens: [DEACTIVATED] Prepared %d custom set threads!!',
				count($ret)
			)
		);
		return $ret;
	...
	protected function workIndexGatherer(array $options) {
		...
		$this->log(
			sprintf(
				'IndexGatherer: Prepared %d set download threads!!',
				count($ret)
			)
		);
		return $ret;
	...
	protected function workIndexConvert(array $options) {
		...
		foreach ($cardList as $card) {
			...
			if (in_array(OracleInfo::getCardTypeName($card), ['Token', 'Emblem'])) {
				$tokenCardList[] = $card;
				/*
				...
				//legality update
				$idTable->updateRowByName(['legality' => $card['legality']], $card['name']);
			}
		}
		...
		$this->log(
			sprintf(
				'IndexConvert: Prepared %d card convert threads!! (%d cards, %d tokens)',
				count($ret),
				count($realCardList),
				count($tokenCardList)
			)
	...
	protected function workIndexImages(array $options) {
		...
		$idTable = $oracle->getIdTable();
		$setList = isset($options['setList'])
			? $options['setList']
			: $idTable->getSetAbbrList();
		...
		$this->log(
			sprintf(
				'IndexImages: Prepared %d image download threads!!',
				count($ret)
			)
		);
		return $ret;
	...
	protected function workIndexPrices(array $options) {
		...
		$this->log(
			sprintf(
				'IndexPrices: Prepared %d set download threads!!',
				count($ret)
			)
		);
		return $ret;
	...
	protected function workDownloadPrices(array $options) {
		...
		foreach ($cardList as $card) {
			if (isset($priceList[$card['name']])) {
				$totalCount++;
				if ($idTable->updateRowById(['price' => $priceList[$card['name']]], $card['id'])) {
					$successCount++;
					$lastCard = $card;
				}
			}
		...
		$this->log(
			sprintf(
				'DownloadPrices: Updated %3d/%3d card prices from %s!!%s',
				$successCount, $totalCount, $options['setName'], $lastCard
			),
			$successCount
		);
	...
	protected function workOracleSet(array $options) {
		...
		foreach ($cardList as $cardId) {
			...
			if (is_array($data)) {
				...
				//'Conspiracy', 
				if (in_array($type, ['Token', 'Emblem', 'Plane', 'Scheme', 'Vanguard', 'Phenomenon'])) {
					continue;
				}
				if ($type === 'Other') {
				...
				if (isset($dataList[$no])) {
					...
					$error = sprintf(
						'ERROR? expansion number already exists: %s %s #%s <> %s #%s',
						$no, $data['name'], $data['oracle_id'], $dataList[$no]['name'], $dataList[$no]['oracle_id']
					);
					//$this->log($error, true);
					continue;
				}
		...
		$this->log(
			sprintf(
				'Gatherer: Imported %3d/%3d cards from %s!!%s',
				$successCount, $totalCount, $options['setName'], $lastCard
			),
			$successCount
		);
	...
	protected function workOracleImport(array $options) {
		...
		$this->log(
			sprintf(
				'MagicDB: Imported %3d/%3d cards from %s!!%s',
				$successCount, $i, $data['expansion_name'], $lastCard
			),
			$successCount
		);
		return $ret;
	...
	protected function workOraclePrice(array $options) {
		...
		while (count($cardList)) {
			...
			if ($price === null) {
				...
				$res = $idTable->updateRowByName(['price' => $price], $name);
				if ($res === null) {
		...
		$this->log(
			sprintf(
				'Priced %3d/%3d cards!!%s',
				$successCount, $totalCount, $lastCard
			),
			$successCount
		);
		return $ret;
	...
	protected function workOracleXML(array $options) {
		...
		$this->log(
			sprintf(
				'OracleXML: Converted %3d/%3d cards!!%s',
				$successCount, $totalCount, $lastCard
			),
			$successCount
		);
	...
	protected function workCustomImport(array $options) {
		...
		$this->log(
			sprintf(
				'CustomImport: Imported %3d/%3d cards!!%s',
				$successCount, $totalCount, $lastCard
			),
			$successCount
		);
	...
	protected function workDownloadImages(array $options) {
		...
		foreach ($options['cardList'] as $card) {
			...
			if ($url = $card['image']) {
				...
				//*/
					...
					if ($file) {
						//$this->log($exportPath);
						if ($file->copyTo($exportDir, $exportName, ['\\Image', 'convertFile'])) {
							$successCount++;
							$lastCard = $card;
						} else {
		...
		foreach ($rarityList as $rarity => $path) {
			$url = sprintf(
				'http://gatherer.wizards.com/Handlers/Image.ashx?type=symbol&set=%s&size=large&rarity=%s',
				isset($abbrExceptionList[$setAbbr]) ? $abbrExceptionList[$setAbbr] : $setAbbr,
				substr($rarity, 0, 1)
			);
		...
		$this->log(
			sprintf(
				'DownloadImages: Converted %3d/%3d images!! [%s]%s',
				$successCount, $totalCount, $setAbbr, $lastCard
			),
	...
	protected function _downloadOracleData($id) {
		...
		static $idBlackList = [
			190199, 201842, 201843, 386322, 159048, 159047, 159056, 209163, 207998, 209162, 197936, 197937, 197261,
			5503, 4979, 5560, 5472, 5607, 5601,
		];
		...
		$ret['type'] = str_replace(['Summon —', 'Eaturecray —', 'Summon - The Biggest, Baddest, Nastiest,', 'Scariest Creature You\'ll Ever See'], 'Creature —', $ret['type']);
		$ret['type'] = str_replace(['Enchant Player'], 'Enchantment — Aura Curse', $ret['type']);
		$ret['type'] = str_replace(['Interrupt'], 'Instant', $ret['type']);
		...
		if (!strlen($ret['expansion_number'])) {
			$this->log(
				sprintf('CARD NUMBER NOT FOUND: %s [%s] #%s%s%s',
					$ret['name'], OracleInfo::getNameKey($ret['name']), $ret['oracle_id'],
					PHP_EOL, print_r($setNumberMap[$abbr], true)
				), true
			);
...
//<card name="Volcanic Geyser" mana="2" type="Instant" set-rarity-name="Magic 2014 Core Set (Uncommon)" set-no="160" type-sup="Instant" set="Magic 2014 Core Set" rarity="Uncommon" href="http://gatherer.wizards.com/Pages/Card/Details.aspx?multiverseid=370614" image="/getResource.php/mtg/cards/Magic2014CoreSet.160" set-rarity="http://gatherer.wizards.com/Pages/Card/../../Handlers/Image.ashx?type=symbol&amp;set=M14&amp;size=small&amp;rarity=U"><mana color="Red" val="2"/></card>
	protected function _convertOracleData(array $data, DOMDocument $doc) {
		...
		if (!isset($data['expansion_list']) or count($data['expansion_list']) > 100) {
			$childList['set'][] = [$data['expansion_name']];
		} else {
			foreach ($data['expansion_list'] as $id => $exp) {
				$childList['set'][] = [$exp['name'], 'sort' => PHP_INT_MAX - $id];
			}
		}
		...
		foreach ($colorList as $key => $val) {
			if ($val > 0) {
				$childList['color'][] = [$key, 'sort' => OracleInfo::getColorIndex($key), 'val' => $val, 'key' => OracleInfo::getColorKey($key)];
			}
		}
