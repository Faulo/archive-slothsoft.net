		...
		if ($res = \Slothsoft\Core\Storage::loadExternalJSON($uri, $cacheTime, json_encode($req), $header)) {
			...
			foreach ($eveList as $eve) {
				if (in_array($eve['EventTypeCode'], $EventTypeCodes)) {
					...
					$startDate = preg_match('/\d+/', $eve['StartDate'], $match)
						? (int) substr($match[0], 0, -3)
						: 0;
		...
		if (preg_match('/(\{.+\})/', $script, $match)) {
			...
			if ($data = json_decode($script, true)) {
				...
				foreach ($data as $events) {
					$month = $events['name'];
					$year = preg_match('/\d{4}/', $month, $match)
						? $match[0]
						: date('Y');
...
foreach (array_merge(['http://locator.wizards.com'], $uriList, $calendarList, $wikiList) as $uri) {
