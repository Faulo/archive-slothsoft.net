...
class Request {
	...
	public static function rateData(array &$NodeArray, $there, BahnRequest $Req) {
	...
	public static function generateRequest(array &$NodeArray, $req) {
		...
		$station_start = array($req['form/station/start'], $req['form/station/end']);
		$station_end = array($req['form/station/end'], $req['form/station/start']);
		$start_date = array($req['form/start/date'], $req['form/end/date']);
		$start_time = array($req['form/start/time'], $req['form/end/time']);
	...
	public static function getRows(DOMElement $Table) {
		...
		$i = 0;
		foreach ($Rows as $Row) {
			if ($Row->parentNode !== $Table) continue;
