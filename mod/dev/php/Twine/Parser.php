...
class Parser {
	...
	protected $utf8BOM;
	protected $weekdayTagList = ['mo', 'tu', 'we', 'th', 'fr'];
	protected $textTagList = [self::TAG_NARRATION, self::TAG_DIALOG, self::TAG_CONTINUE];
	protected $classTagList = [self::TAG_DIALOG, self::TAG_POINTS];
	...
	protected function createStartTag($tag) {
		$class = in_array($tag, $this->classTagList)
			? ' class="npc-?"'
			: '';
