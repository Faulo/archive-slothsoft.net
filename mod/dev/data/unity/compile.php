	...
	foreach ($targets as $target) {
		...
		if (is_dir($projectDir)) {
			...
			if (preg_match('~m_EditorVersion: (.+)~', $unityVersion, $match)) {
				...
				$cmd = sprintf(
					'%s -quit -accept-apiupdate -batchmode -nographics -logFile %s -projectPath %s -executeMethod %s %s',
					$unity,
					escapeshellarg($logFile),
					escapeshellarg($projectDir),
					escapeshellarg('Slothsoft.UnityExtensions.Editor.Build.'.$target['platform']),
					escapeshellarg($outputDir.'/'.$target['file'])
				). PHP_EOL;
				echo $cmd;
				...
				if (isset($target['backup']) and is_dir($target['backup'])) {
					...
					$cmd = sprintf(
						...
						escapeshellarg($target['backup'] . $localFolder)
					). PHP_EOL;
					echo $cmd;
					file_put_contents($queue, $cmd, FILE_APPEND |  LOCK_EX);
				}
